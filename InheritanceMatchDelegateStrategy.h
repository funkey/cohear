#ifndef SR_INHERITANCE_MATCH_DELEGATE_STRATEGY_H__
#define SR_INHERITANCE_MATCH_DELEGATE_STRATEGY_H__
...
public:
	...
	struct DelegateConverter {
		...
		static Delegate<ToType> Convert(Delegate<FromType>& from) {

			// The following code assumes that the memory layout of 
			// Delegate<ToType> and Delegate<FromType> is the same (which it is, 
			// unless someone specializes Delegate<T> [their own fault]), and 
			// that Delegate<ToType>::operator() is doing what we would expect.  
			// The latter is probably guarnateed by the C0x-standard (see 
			// below), but at least true in practice for a lot of 
			// compilers/architectures. For instance, GLib is doing much more 
			// evil casts without problems (see 
			...
			return *reinterpret_cast<Delegate<ToType>*>(&from);
		}
	};

	static bool GetDelegate(CallbackDescription& cd, Delegate<SignalType>& delegate) {

		Delegate<AsSignalType> uncastDelegate;
		bool foundMatch = ExactMatchDelegateStrategy<AsSignalType>::GetDelegate(cd, uncastDelegate);

		if (foundMatch) {

			delegate = DelegateConverter<AsSignalType, SignalType>::Convert(uncastDelegate);
			return true;
		}

		typedef typename SignalTraits<AsSignalType>::parent_type ParentType;
		return InheritanceMatchDelegateStrategy<SignalType, ParentType>::GetDelegate(cd, delegate);
	}
};
...
public:
	...
	static bool GetDelegate(CallbackDescription&, Delegate<SignalType>&) {
